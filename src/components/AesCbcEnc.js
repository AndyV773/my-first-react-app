...
import { aesCbcEncrypt, uint8ToBase64, textEncoder } from "../utils/cryptoUtils";


const AesCbcEnc = ({ showMsg, theme, onToggleTheme }) => {
  ...
  const handleEncrypt = async () => {
    const output = aesCbcEncrypt(input, aesKey); // fileInput in Uint8Array
    if (output.error) {
  ...
  const handleSaveFile = () => {
    if (!input) return showMsg("Nothing to save.", true);
    saveFileAsEc(input);
  }
     ...
     <main className="container">
        <nav>
          <div className="flex g1">
            <Link to="/">Home</Link>
            <Link to="/aes-cbc-dec">Decode</Link>
          </div>
