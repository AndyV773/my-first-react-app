...
import { textEncoder, sha256, xorEncoder, generateSaltBytes, uint8ToBase64 } from "../utils/cryptoUtils";
...
const XorBasedEnc = ({ showMsg, theme, onToggleTheme }) => {
  ...
  const handleXorEncoder = async () => {
    ...
    for (let i = 0; i < iterations; i ++ ) {
        ...
        console.log('com',combined.length)

        const hash = await sha256(combined);

        totalHash.push(hash);

        encrypted = xorEncoder(combined, hash);

        console.log("Encrypted:", Array.from(encrypted.slice(0, 16)));
